...
// List Product of a cart
	 ...
	 @Query("select DISTINCT p from Product p "
             + "join Cart c "
             + "on c.id=:cart_id")
	public List<Product> getCartProducts(@Param("cart_id")Long cart_id);

	/*Delete CartItem
		@Modifying
		@Transactional
		@Query("select orderline from Cart c as ol "
				+ "where id=:cart_id")
			//	+ "and u.id=:user_id")
		public void RemoveOrderLines(Long cart_id);*/
		
		
	 @Query("select DISTINCT c from Cart c "
	             + "where c.id=:cart_id")
	public <List> Cart getAllOrderLinesByCart(@Param("cart_id")Long cart_id);
